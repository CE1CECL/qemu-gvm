...
DECLARE_INSTANCE_CHECKER(IntelHDAState, INTEL_HDA, TYPE_INTEL_HDA_GENERIC)
...
static void intel_hda_notify_codecs(IntelHDAState *d, uint32_t stream,
...
static void intel_hda_set_int_ctl(IntelHDAState *d, const IntelHDAReg *reg,
                                  uint32_t old) {
  intel_hda_update_irq(d);
}
...
static void intel_hda_set_corb_wp(IntelHDAState *d, const IntelHDAReg *reg,
                                  uint32_t old) {
  intel_hda_corb_run(d);
}

static void intel_hda_set_corb_ctl(IntelHDAState *d, const IntelHDAReg *reg,
                                   uint32_t old) {
  intel_hda_corb_run(d);
}

static void intel_hda_set_rirb_wp(IntelHDAState *d, const IntelHDAReg *reg,
                                  uint32_t old) {
...
static void intel_hda_set_rirb_sts(IntelHDAState *d, const IntelHDAReg *reg,
                                   uint32_t old) {
  intel_hda_update_irq(d);
...
static void intel_hda_set_ics(IntelHDAState *d, const IntelHDAReg *reg,
                              uint32_t old) {
  if (d->ics & ICH6_IRS_BUSY) {
    intel_hda_corb_run(d);
  }
...
#define HDA_STREAM(_t, _i)                                                     \
  [ST_REG(_i, ICH6_REG_SD_CTL)] =                                              \
    ...
    HDA_STREAM("IN", 0) HDA_STREAM("IN", 1) HDA_STREAM("IN", 2)
        HDA_STREAM("IN", 3)

            HDA_STREAM("OUT", 4) HDA_STREAM("OUT", 5) HDA_STREAM("OUT", 6)
                HDA_STREAM("OUT", 7)
...
static void intel_hda_reg_write(IntelHDAState *d, const IntelHDAReg *reg,
                                uint32_t val, uint32_t wmask) {
  uint32_t *addr;
  uint32_t old;
...
static void intel_hda_realize(PCIDevice *pci, Error **errp) {
  ...
  memory_region_init(&d->container, OBJECT(d), "intel-hda-container", 0x4000);
  memory_region_init_io(&d->mmio, OBJECT(d), &intel_hda_mmio_ops, d,
                        "intel-hda", 0x2000);
  memory_region_add_subregion(&d->container, 0x0000, &d->mmio);
  memory_region_init_alias(&d->alias, OBJECT(d), "intel-hda-alias", &d->mmio, 0,
                           0x2000);
...
static const VMStateDescription vmstate_intel_hda_stream = {
    ...
    .version_id = 1,
    .fields = (VMStateField[]){VMSTATE_UINT32(ctl, IntelHDAStream),
...
static const VMStateDescription vmstate_intel_hda = {
    ...
    .fields = (VMStateField[]){
        ...
        VMSTATE_UINT32(icw, IntelHDAState), VMSTATE_UINT32(irr, IntelHDAState),
        VMSTATE_UINT32(ics, IntelHDAState),
        VMSTATE_STRUCT_ARRAY(st, IntelHDAState, 8, 0, vmstate_intel_hda_stream,
                             IntelHDAStream),
